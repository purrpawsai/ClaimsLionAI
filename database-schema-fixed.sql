-- Database schema for queue-based processing system (RENAMED to avoid conflicts)
...
-- Queue analysis results table (renamed to avoid conflict)
CREATE TABLE IF NOT EXISTS queue_analysis_results (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_queue_analysis_results_queue_id ON queue_analysis_results(queue_id);
...
CREATE TRIGGER update_queue_analysis_results_updated_at 
    BEFORE UPDATE ON queue_analysis_results 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- RLS (Row Level Security) policies (optional - adjust based on your auth needs)
-- ALTER TABLE processing_queue ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE queue_analysis_results ENABLE ROW LEVEL SECURITY;
